#ifndef LANGUAGETERMS_H
#define LANGUAGETERMS_H

#include <QStringList>
#include <QVector>

extern const QStringList CPlusPlusTerms;
extern const QStringList PythonTerms;
extern const QStringList BashTerms;
extern const QStringList JavaTerms;
extern const QStringList JavaScriptTerms;
extern const QStringList AssemblyTerms;
extern const QStringList CTerms;
extern const QStringList RustTerms;
extern const QStringList FortranTerms;
extern const QStringList PhpTerms;
extern const QStringList RubyTerms;
extern const QStringList PerlTerms;
extern const QVector<QStringList> AllTerms;
extern QStringList correctValueTexts;
extern const QStringList Titles;
extern const std::unordered_map<std::string, std::string> explanationMap;

#endif // LANGUAGETERMS_H
